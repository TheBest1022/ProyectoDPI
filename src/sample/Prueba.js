export const dataPruebas = [
  {
    id: "Animales",
    Img: [
      {
        id: "1",
        img: {
          uri: "https://www.dropbox.com/s/kzsbi2wgo6xskcn/animalespicto.png?dl=1",
        },
        descripcion: "Los Animales",
      },
    ],
  },
  {
    id: "Frutas",
    Img: [
      {
        id: "2",
        img: {
          uri: "https://www.dropbox.com/s/novffr7xp6yzwyz/fruta.png?dl=1",
        },
        descripcion: "Las Frutas",
      },
    ],
  },
  {
    id: "Verduras",
    Img: [
      {
        id: "3",
        img: {
          uri: "https://www.dropbox.com/s/7w3uoafjkvc54qb/verdura.png?dl=1",
        },
        descripcion: "Las Verduras",
      },
    ],
  },
  {
    id: "Colores",
    Img: [
      {
        id: "4",
        img: {
          uri: "https://www.dropbox.com/s/s87pg78iv4jgvja/rojo.png?dl=1",
        },
        descripcion: "Los Colores",
      },
    ],
  },
  {
    id: "Familia",
    Img: [
      {
        id: "5",
        img: {
          uri: "https://www.dropbox.com/s/rqzm58gcck0hlb5/pap%C3%A1.png?dl=1",
        },
        descripcion: "La Familia",
      },
    ],
  },
  {
    id: "Emociones",
    Img: [
      {
        id: "6",
        img: {
          uri: "https://www.dropbox.com/s/xg8dmzqpwlnq0e6/alegre.png?dl=1",
        },
        descripcion: "Las Emociones",
      },
    ],
  },
  {
    id: "Cuerpo",
    Img: [
      {
        id: "7",
        img: {
          uri: "https://www.dropbox.com/s/74kvp8au94kmbn1/humano.png?dl=1",
        },
        descripcion: "El cuerpo",
      },
    ],
  },
  {
    id: "Sentidos",
    Img: [
      {
        id: "7",
        img: {
          uri: "https://www.dropbox.com/s/3qwy6a1z1fgzrau/sentidos.png?dl=1",
        },
        descripcion: "Los Sentidos",
      },
    ],
  },
  {
    id: "Numeros",
    Img: [
      {
        id: "8",
        img: {
          uri: "https://www.dropbox.com/s/ds41hg8pidqrp46/manouno.png?dl=1",
        },
        descripcion: "Los Números",
      },
    ],
  },
  {
    id: "Normas",
    Img: [
      {
        id: "9",
        img: {
          uri: "https://www.dropbox.com/s/5rhbxlpquavsky4/respetar.jpg?dl=1",
        },
        descripcion: "Las Normas de Convicencia",
      },
    ],
  },
  {
    id: "Oficio",
    Img: [
      {
        id: "10",
        img: {
          uri: "https://www.dropbox.com/s/7mp8an5gqs0xsya/policia.png?dl=1",
        },
        descripcion: "Los Oficios",
      },
    ],
  },
  {
    id: "Estaciones",
    Img: [
      {
        id: "11",
        img: {
          uri: "https://www.dropbox.com/s/07rt0kq13sgc5ml/estacion.png?dl=1",
        },
        descripcion: "Las Estaciones del año",
      },
    ],
  },
  {
    id: "Habitaciones",
    Img: [
      {
        id: "13",
        img: {
          uri: "https://www.dropbox.com/s/uhhihaqkeqd51vg/cocina.png?dl=1",
        },
        descripcion: "Las Habitaciones de la Casa",
      },
    ],
  },
  {
    id: "Transporte",
    Img: [
      {
        id: "14",
        img: {
          uri: "https://www.dropbox.com/s/omc4ewdsd29443j/transporte.png?dl=1",
        },
        descripcion: "Los Medios de Transporte",
      },
    ],
  },
  {
    id: "Rostro",
    Img: [
      {
        id: "15",
        img: {
          uri: "https://www.dropbox.com/s/ir0f330qdk50jjk/cara.png?dl=1",
        },
        descripcion: "El Rostro",
      },
    ],
  },
  {
    id: "Sabores",
    Img: [
      {
        id: "16",
        img: {
          uri: "https://www.dropbox.com/s/o33s83kex1474uu/sabore.png?dl=1",
        },
        descripcion: "Los Sabores",
      },
    ],
  },
  {
    id: "Escolares",
    Img: [
      {
        id: "17",
        img: {
          uri: "https://www.dropbox.com/s/oja15zoxyzfjw1b/lapiz.png?dl=1",
        },
        descripcion: "Los útiles escolares",
      },
    ],
  },
  {
    id: "Aseo",
    Img: [
      {
        id: "18",
        img: {
          uri: "https://www.dropbox.com/s/pvjdivi1ddah8ir/utiles.png?dl=1",
        },
        descripcion: "Utiles de Aseo",
      },
    ],
  },
  {
    id: "Peligroso",
    Img: [
      {
        id: "19",
        img: {
          uri: "https://www.dropbox.com/s/5pvvlbtmnfxbhvg/cuchillo.png?dl=1",
        },
        descripcion: "Objetos Peligrosos",
      },
    ],
  },
  {
    id: "Semáforo",
    Img: [
      {
        id: "20",
        img: {
          uri: "https://www.dropbox.com/s/uv2pegc8wmxp82u/semaforo.png?dl=1",
        },
        descripcion: "El Semáforo",
      },
    ],
  },
  {
    id: "Espaciales",
    Img: [
      {
        id: "20",
        img: {
          uri: "https://www.dropbox.com/s/jawy5e7t9wvvtla/conceptosespaciales.png?dl=1",
        },
        descripcion: "Conceptos Espaciles",
      },
    ],
  },
  {
    id: "Geometricas",
    Img: [
      {
        id: "21",
        img: {
          uri: "https://www.dropbox.com/s/1swf4t5b7tuifau/figurasgeometricas.png?dl=1",
        },
        descripcion: "Figuras Geométricas",
      },
    ],
  },
];

export const dataPreguntas = [
  {
    id: "Animales",
    Question: [
      {
        id: "1",
        pregunta: "¿Qué Animal es un águila?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/vb9regzyyye4lhc/vaca.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/9r0npdb3wic27ua/aguila.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/t30tdnfpfsv63d6/sapo.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Qué Animal es un Tiburón?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/fqditoln15e3200/tiburon.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/nbzkyr9htobu6fr/hiena.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/78os7dla5suuv0c/pantera.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Qué Animal es un zorro?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/5chpsvok365huk8/canguro.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/7919irc9rlqrm1k/oveja.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/bm9e13zfz3jlxa6/zorro.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Qué Animal es un Koala?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/8rn8mbl2qjv44i0/Koala.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/xrb909uxp22l59t/lobo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/krfjao0ed4vz30b/panda.png?dl=1",
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Qué Animal es Elefante?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/lbocdbt5kr31vf4/reno.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/bhl8cx0iev4l6dp/delfin.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/5n1rwj966uglobx/elefante.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "6",
        pregunta: "¿Qué Animal es una Cebra?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/7919irc9rlqrm1k/oveja.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/b75dqwqpbmvl6po/cebra.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/nolibhu8zviogpq/anguila.png?dl=1",
          },
        ],
      },
      {
        id: "7",
        pregunta: "¿Qué Animal tiene una trompa?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/neau64604ujodfq/hipopotamo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/rfl6cy66ntv7w2b/llama.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/5n1rwj966uglobx/elefante.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "8",
        pregunta: "¿Qué Animal tiene un cuello largo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/39uewhrq83albhr/jirafa.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/mrv4kf75pfn2hci/loro.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/i3hlilwn9dd4m1a/conejo.png?dl=1",
          },
        ],
      },
      {
        id: "9",
        pregunta: "¿Qué Animal tiene 8 patas?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/fw4jik6nykjvg1w/ara%C3%B1a.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/rv8f0lb6tk1ns5q/caballo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/0giym1p50as39jg/foca.png?dl=1",
          },
        ],
      },
      {
        id: "10",
        pregunta: "¿Qué Animal vuela?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/x4r60yalsgz49rr/paloma.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/gnpj9zkyqgvhdjv/pez.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ylrnsxftm8v9p2y/toro.png?dl=1",
          },
        ],
      },
    ],
  },
  {
    id: "Frutas",
    Question: [
      {
        id: "1",
        pregunta: "¿Qué fruta es una manzana?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/kab2cgxht0uptl9/melon.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/2cpxji1xxp7fchl/tomate.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/jtbza8zl5zyhrhr/manzana.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Qué fruta es un plátano?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/symee8zz5qiu11p/pera.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/d47l8d80c9byyab/platano.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/mrav8a4llkolzzd/kiwi.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Qué fruta es una ciruela?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/k7kgysk27m0qxau/ciruela.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/3s0k9wd4koxv75e/pi%C3%B1a.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/k5sl4ihq6hsxplr/fresa.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Qué fruta es una cereza?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/kbv18ym45pnxcab/zapallo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/xzhu1fk999jkzcf/cereza.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/y4zlhvzmyx4lgh8/mandarina.png?dl=1",
          },
        ],
      },
      {
        id: "5",
        pregunta: "¿Qué fruta es una pera?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/symee8zz5qiu11p/pera.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/0ul3nqjtusl3f1r/frambuesa.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/frhnevfsr7qhf8g/uvas.png?dl=1",
          },
        ],
      },
      {
        id: "6",
        pregunta: "¿Cuál no es una fruta?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/4xuypghmh2lw86y/higo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/l31w1gwyy3jqfla/cebolla.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/kab2cgxht0uptl9/melon.png?dl=1",
          },
        ],
      },
      {
        id: "7",
        pregunta:
          "Blanca por dentro, verde por fuera. Si quieres que te diga, espera.",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3s0k9wd4koxv75e/pi%C3%B1a.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/y4zlhvzmyx4lgh8/mandarina.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/symee8zz5qiu11p/pera.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "8",
        pregunta:
          "Tengo duro el corazón, pulpa blanca y líquido en mi interior",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/k5sl4ihq6hsxplr/fresa.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/v8kfag9ac15un2d/coco.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/z4plyd2pl6onn0b/sandia.png?dl=1",
          },
        ],
      },
      {
        id: "9",
        pregunta:
          "De cuerpo espinado y bien coronado, soy delicioso bocado a boca.",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/kab2cgxht0uptl9/melon.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/3s0k9wd4koxv75e/pi%C3%B1a.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/k2f21so58kbdzm1/lechuga.png?dl=1",
          },
        ],
      },
      {
        id: "10",
        pregunta:
          "Fruta pequeña de color rojo y dulce, gusto mucho a niños y mayores.",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/f7wasdh3d3glt36/arandanos.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/4npnc4014y8fswa/chirimoya.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/k5sl4ihq6hsxplr/fresa.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
    ],
  },
  {
    id: "Verduras",
    Question: [
      {
        id: "1",
        pregunta: "¿Qué verdura es tomate?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/0k2m3343ooats3f/apio.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/2cpxji1xxp7fchl/tomate.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/l31w1gwyy3jqfla/cebolla.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Qué verdura es una zanahoria?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/tsbtels82ynxvbt/pepino.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/br4441kg2pavust/coliflor.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/q01s3m73ezvacbh/zanahoria.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Qué verdura es una lechuga?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/k2f21so58kbdzm1/lechuga.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/vmegvd02yu00hlt/alcachofa.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/62ryme9k3burwom/pimiento.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Cúal no es una verdura?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/tsbtels82ynxvbt/pepino.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/xzhu1fk999jkzcf/cereza.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/1xrdmm62kl2oxti/brocoli.png?dl=1",
          },
        ],
      },
      {
        id: "5",
        pregunta:
          "Vive bajo tierra, muere en la sartén, sus diez camisas llorando se ven.",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/5x60pwo70wcg0em/berenjena.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/l31w1gwyy3jqfla/cebolla.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/5xn34s6e7cm8zbj/champi%C3%B1ones.png?dl=1",
          },
        ],
      },
      {
        id: "6",
        pregunta:
          "No toma té, ni toma café y colorado está. ¿Sabrías decirme quién es?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/hxxdsazsppe3gic/espinaca.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/y4zlhvzmyx4lgh8/mandarina.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/2cpxji1xxp7fchl/tomate.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "7",
        pregunta: "Soy un vegetal redondo y grande útil en ensaladas.",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/k2f21so58kbdzm1/lechuga.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/v8kfag9ac15un2d/coco.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/tsbtels82ynxvbt/pepino.png?dl=1",
          },
        ],
      },
      {
        id: "8",
        pregunta: "Flaca y larga, lleva gorro verde y camisa naranja.",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/f7wasdh3d3glt36/arandanos.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/q01s3m73ezvacbh/zanahoria.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/k5sl4ihq6hsxplr/fresa.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
    ],
  },
  {
    id: "Colores",
    Question: [
      {
        id: "1",
        pregunta: "¿Cúal es el color rojo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/a63ndkjakloty24/azul.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/s87pg78iv4jgvja/rojo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/9gavmowdfaqfc3y/amarillo.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Cúal es el color azul?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/cttndie6cn7200b/morado.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/3temsoorp5rdv3o/verde.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/a63ndkjakloty24/azul.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Cúal es el color amarillo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/9gavmowdfaqfc3y/amarillo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/5mvqfbjb2v0w6oh/blanco.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/iwmiggszfxdgcul/negro.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Cúal es el color verde?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ewae74avubzevgl/gris.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/s5c3coz4q5ncwc4/marr%C3%B3n.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/3temsoorp5rdv3o/verde.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "5",
        pregunta: "¿Cúal es el color rosado?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/kykvfn4ep4jhxdr/rosado.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/8g8qms1d5fuyg4v/plateado.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/etjs84iyqkcgx4i/dorado.png?dl=1",
          },
        ],
      },
      {
        id: "6",
        pregunta: "¿Cúal es color morado?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/6yo8kxhskjcvwyk/anaranjado.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/cttndie6cn7200b/morado.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/s5c3coz4q5ncwc4/marr%C3%B3n.png?dl=1",
          },
        ],
      },
      {
        id: "7",
        pregunta: "¿Cúal es el color blanco?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/5mvqfbjb2v0w6oh/blanco.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/iwmiggszfxdgcul/negro.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/3temsoorp5rdv3o/verde.png?dl=1",
          },
        ],
      },
      {
        id: "8",
        pregunta: "¿Cúal es color negro?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ewae74avubzevgl/gris.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/s5c3coz4q5ncwc4/marr%C3%B3n.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/iwmiggszfxdgcul/negro.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "9",
        pregunta: "¿Cúal es color anaranjado?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/6yo8kxhskjcvwyk/anaranjado.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/84ae37bn5qsuehy/granate.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/9gavmowdfaqfc3y/amarillo.png?dl=1",
          },
        ],
      },
      {
        id: "10",
        pregunta: "¿Cúal es color gris?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/cttndie6cn7200b/morado.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ewae74avubzevgl/gris.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/3temsoorp5rdv3o/verde.png?dl=1",
          },
        ],
      },
    ],
  },
  {
    id: "Familia",
    Question: [
      {
        id: "1",
        pregunta: "¿Quién es el papá?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ckl3adob3dvtsf0/mam%C3%A1.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/z4g4y6g6uzmy42y/hermana.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/rqzm58gcck0hlb5/pap%C3%A1.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Quién es la mamá?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/yo978vbjqhufwa1/abuelo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/f3bljlbklnsbz9q/tio.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ckl3adob3dvtsf0/mam%C3%A1.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Quién es la hermana?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/jilsgeaze45j8a1/abuela.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/z4g4y6g6uzmy42y/hermana.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/3jpfe7b67erhz4i/tia.jpg?dl=1",
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Quién es el abuelo",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/yo978vbjqhufwa1/abuelo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/f3bljlbklnsbz9q/tio.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/jilsgeaze45j8a1/abuela.png?dl=1",
          },
        ],
      },
      {
        id: "5",
        pregunta: "¿Quién es la abuela?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ckl3adob3dvtsf0/mam%C3%A1.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/jilsgeaze45j8a1/abuela.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/3jpfe7b67erhz4i/tia.jpg?dl=1",
          },
        ],
      },
    ],
  },
  {
    id: "Emociones",
    Question: [
      {
        id: "1",
        pregunta: "¿Cuál es la cara feliz?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/xg8dmzqpwlnq0e6/alegre.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/8al8474yxwdrpjl/triste.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/caxu1km66gjfu3h/enfadado.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Cuál es la cara triste?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/8al8474yxwdrpjl/triste.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/caxu1km66gjfu3h/enfadado.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/xg8dmzqpwlnq0e6/alegre.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Cuál es la cara enojada?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/xg8dmzqpwlnq0e6/alegre.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/8al8474yxwdrpjl/triste.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/caxu1km66gjfu3h/enfadado.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Cúal es la cara aburrido?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/fupunmocb244elc/aburrido.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/l9iw65cjtuxh5et/serio.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/7gdm4dusxvn7h25/avergonzado.png?dl=1",
          },
        ],
      },
      {
        id: "5",
        pregunta: "¿Cúal es la cara de miedo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/xeqpgkczktpydn8/cansado.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/hflhhsjcn3b7mlk/miedoso.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/strn9pfzthl9k20/confuso.png?dl=1",
          },
        ],
      },
      {
        id: "6",
        pregunta: "¿Cúal es la cara de sorpresa?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/hktarnzipzk5pfw/sorprendido.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/6p8ndqmn0faozlb/nervioso.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/fewhvcrcy9rz6yc/t%C3%ADmido.png?dl=1",
          },
        ],
      },
      {
        id: "7",
        pregunta: "¿Cúal es la cara de avergonzado?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/strn9pfzthl9k20/confuso.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/xg8dmzqpwlnq0e6/alegre.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/7gdm4dusxvn7h25/avergonzado.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "8",
        pregunta: "¿Cúal es la cara de cansancio?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/hktarnzipzk5pfw/sorprendido.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/xeqpgkczktpydn8/cansado.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/2k7vbfpd55ngq3j/atento.png?dl=1",
          },
        ],
      },
      {
        id: "9",
        pregunta: "¿Cúal es la cara de confundido?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/fupunmocb244elc/aburrido.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/strn9pfzthl9k20/confuso.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/kuhr8lh4m8c76lf/distraido.png?dl=1",
          },
        ],
      },
      {
        id: "10",
        pregunta: "¿Cúal es la cara de distraído?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/kuhr8lh4m8c76lf/distraido.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/uk8jtse3w9ko5pg/Goloso.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/6p8ndqmn0faozlb/nervioso.png?dl=1",
          },
        ],
      },
      {
        id: "11",
        pregunta: "¿Cúal es la cara de serio?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/2k7vbfpd55ngq3j/atento.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/l9iw65cjtuxh5et/serio.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/caxu1km66gjfu3h/enfadado.png?dl=1",
          },
        ],
      },
      {
        id: "12",
        pregunta: "¿Cúal es la cara de tímido?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/fewhvcrcy9rz6yc/t%C3%ADmido.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/strn9pfzthl9k20/confuso.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/8al8474yxwdrpjl/triste.png?dl=1",
          },
        ],
      },
    ],
  },
  {
    id: "Cuerpo",
    Question: [
      {
        id: "1",
        pregunta: "¿Cuál es la cabeza?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/mj2a0w5m5d5s4ny/cabeza.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/nqn5fpxjc2dgm7f/cuello.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/9rcl9wumncqc4m5/dientes.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Cuál es el cuello?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/rdh2dpxwpj9hmzo/ojos.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/yz5j7p55lxy29lr/oreja.jpg?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/nqn5fpxjc2dgm7f/cuello.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Cuál es la boca?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/fgl7fl9z9z70pqp/bocas.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/48s9tucmyhplg0i/manos.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/o9z64bod41g495q/pie.png?dl=1",
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Cuál es la lengua?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/r7g4fc9faj1be88/labios.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/umfxkgsu4vetnsw/lengua.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/jeacy5zz9xsikis/muslo.png?dl=1",
          },
        ],
      },
      {
        id: "5",
        pregunta: "¿Cúales son los labios?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3s8oyq0s4f3zv9r/barriga.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/dsxz58lavmfny8i/mejilla.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/r7g4fc9faj1be88/labios.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "6",
        pregunta: "¿Cúal es la nariz?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/1h9e19x0vb6w4mo/narizon.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/t8vjxu4ygksefz3/barbilla.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/qubi730cqrejzmz/frente.png?dl=1",
          },
        ],
      },
      {
        id: "7",
        pregunta: "¿Cuales son los dientes?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/9rcl9wumncqc4m5/dientes.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/umfxkgsu4vetnsw/lengua.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/duv0rpiijki9sr3/dedo.png?dl=1",
          },
        ],
      },
      {
        id: "8",
        pregunta: "¿Cuales son los ojos?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/qubi730cqrejzmz/frente.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/rdh2dpxwpj9hmzo/ojos.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/3rp2cq483etri3p/pierna.jpg?dl=1",
          },
        ],
      },
      {
        id: "9",
        pregunta: "¿Cúal es la oreja?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/yz5j7p55lxy29lr/oreja.jpg?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/sfcx6f46nwh0wfl/pesta%C3%B1a.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/xokh45kk0wap5g1/pecho.jpg?dl=1",
          },
        ],
      },
      {
        id: "10",
        pregunta: "¿Cúal es el pecho?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/xokh45kk0wap5g1/pecho.jpg?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/x1eehafiq2yzwp0/espalda.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/cjfawgyhbke4r40/Rodilla.png?dl=1",
          },
        ],
      },
      {
        id: "11",
        pregunta: "¿Cúal es la espalda?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/xokh45kk0wap5g1/pecho.jpg?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/x1eehafiq2yzwp0/espalda.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/cjfawgyhbke4r40/Rodilla.png?dl=1",
          },
        ],
      },
      {
        id: "12",
        pregunta: "¿Cúal es el brazo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/c53xu9we37rb9i9/brazo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/3rp2cq483etri3p/pierna.jpg?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/yoei3hfyvlnzf2p/codo.png?dl=1",
          },
        ],
      },
      {
        id: "13",
        pregunta: "¿Cúal es un dedo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/duv0rpiijki9sr3/dedo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/fiql3d1ybyrrd6e/mu%C3%B1eca.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/rsiuziriktj9na0/tobillo.png?dl=1",
          },
        ],
      },
      {
        id: "14",
        pregunta: "¿Cúal es la mano?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/48s9tucmyhplg0i/manos.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/o9z64bod41g495q/pie.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/3s8oyq0s4f3zv9r/barriga.png?dl=1",
          },
        ],
      },
      {
        id: "15",
        pregunta: "¿Cúal es el codo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/yoei3hfyvlnzf2p/codo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/cjfawgyhbke4r40/Rodilla.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/90vkpycs5mnks8q/talon.png?dl=1",
          },
        ],
      },
      {
        id: "16",
        pregunta: "¿Cúal la muñeca?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/fiql3d1ybyrrd6e/mu%C3%B1eca.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/mj2a0w5m5d5s4ny/cabeza.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/fgl7fl9z9z70pqp/bocas.png?dl=1",
          },
        ],
      },
      {
        id: "17",
        pregunta: "¿Cúal es la pierna?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/o9z64bod41g495q/pie.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/3rp2cq483etri3p/pierna.jpg?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/90vkpycs5mnks8q/talon.png?dl=1",
          },
        ],
      },
      {
        id: "18",
        pregunta: "¿Cúal es la rodilla?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/cjfawgyhbke4r40/Rodilla.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/48s9tucmyhplg0i/manos.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/nwlpmnny858st7y/hombro.png?dl=1",
          },
        ],
      },
      {
        id: "19",
        pregunta: "¿Cúal es el hombro?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/xokh45kk0wap5g1/pecho.jpg?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/nwlpmnny858st7y/hombro.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/mj2a0w5m5d5s4ny/cabeza.png?dl=1",
          },
        ],
      },
      {
        id: "20",
        pregunta: "¿Cúal es el tobillo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/rsiuziriktj9na0/tobillo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/jeacy5zz9xsikis/muslo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/4blxakvhgs2s3hk/u%C3%B1a.png?dl=1",
          },
        ],
      },
      {
        id: "19",
        pregunta: "¿Cúal es el talón?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/x1eehafiq2yzwp0/espalda.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/t8vjxu4ygksefz3/barbilla.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/90vkpycs5mnks8q/talon.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
    ],
  },
  {
    id: "Numeros",
    Question: [
      {
        id: "1",
        pregunta: "¿Cual es el número 1?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ds41hg8pidqrp46/manouno.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/0apubtffv6r9kh8/manodos.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/phc0457jwaw6fbj/manotres.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Cúal es el número 2?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/phc0457jwaw6fbj/manotres.png?dl=1",
          },
          {
            uri: " https://www.dropbox.com/s/0apubtffv6r9kh8/manodos.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/ds41hg8pidqrp46/manouno.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Cúal es el número 3?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/0apubtffv6r9kh8/manodos.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ds41hg8pidqrp46/manouno.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/phc0457jwaw6fbj/manotres.png?dl=1 ",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Cúal es el número 4?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/11lfcvvxe626qo4/manocinco.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/huz0a4a048un2gw/manoseis.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/gaiem0filypciuz/manocuatro.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "5",
        pregunta: "¿Cúal es el número 5?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/huz0a4a048un2gw/manoseis.png?dl=1",
          },
          {
            uri: " https://www.dropbox.com/s/11lfcvvxe626qo4/manocinco.png?dl=1 ",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/gaiem0filypciuz/manocuatro.png?dl=1",
          },
        ],
      },
      {
        id: "6",
        pregunta: "¿Cúal es el número 6?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/gaiem0filypciuz/manocuatro.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/11lfcvvxe626qo4/manocinco.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/huz0a4a048un2gw/manoseis.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "7",
        pregunta: "¿Cúal es el número 7?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/y55cw2w2ibbn9a9/manosiete.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: " https://www.dropbox.com/s/ra50dwclud5496t/manonueve.png?dl=1 ",
          },
          {
            uri: "https://www.dropbox.com/s/y4ead6izpt51naq/manoOcho.png?dl=1",
          },
        ],
      },
      {
        id: "8",
        pregunta: "¿Cúal es el número 8?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/y55cw2w2ibbn9a9/manosiete.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/y4ead6izpt51naq/manoOcho.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/ra50dwclud5496t/manonueve.png?dl=1",
          },
        ],
      },
      {
        id: "9",
        pregunta: "¿Cúal es el número 9?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ra50dwclud5496t/manonueve.png?dl=1 ",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/y4ead6izpt51naq/manoOcho.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/y55cw2w2ibbn9a9/manosiete.png?dl=1",
          },
        ],
      },
      {
        id: "10",
        pregunta: "¿Cúal es el número 10?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/11lfcvvxe626qo4/manocinco.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/y55cw2w2ibbn9a9/manosiete.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/vh3tfl5uihom1jb/manodiez.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
    ],
  },
  {
    id: "Normas",
    Question: [
      {
        id: "1",
        pregunta: "¿Qué imagen representa el respeto?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/7a00srlcelkka8a/pellizcar.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/3bmg8xr3hnqfs92/saludar.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/v1ngikcepgg042o/morder.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Que imagen representa la solidaridad?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/rovnu2t1pn0qmdc/ayudar.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/xxpjclxp6g38u78/pelear.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/h6fcvtb3t2y6q5s/ara%C3%B1ar.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Que imagen representa el cariño?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/kqzz20heztlvf76/discutir.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/pk20ufdpn4avcy7/querer.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/936jzjianegh1lu/escupir.png?dl=1",
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Que imagen representa la empatía?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/k8mrqzjpvwyp9at/patada.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/x2bp61w5zww68pt/burlarse.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/8x1azykizce3bzl/empatia.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "5",
        pregunta: "¿Que imagen representa la unión?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ca2afnv3sdxsib3/robar.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/zqnuti2jfwi17eh/amigo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/aksbbqotjcqie14/molestar.png?dl=1",
          },
        ],
      },
    ],
  },
  {
    id: "Oficio",
    Question: [
      {
        id: "1",
        pregunta: "¿Qué personaje es un policía?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/edkuf3jyt2phbda/bombero.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/7mp8an5gqs0xsya/policia.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/9jr2vtawk1amvib/enfermera.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Qué personaje es una médico?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ve9neywx676xrll/medico.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/qdgj3277oc5wn23/ingeniero.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/cbqk03aig8w5hpz/alba%C3%B1il.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Qué personaje es un bombero?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/e0cwyspav2lzwu7/maestra.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/edkuf3jyt2phbda/bombero.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/ul8f4tfi67neiw8/abogado.png?dl=1",
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Qué personaje es un sastre o costurero?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/zoduuho2xls958y/sastre.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/ghfmqf6m0iisfq0/agricultor.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/uyao5yvfvmnxcyo/alcalde.png?dl=1",
          },
        ],
      },
      {
        id: "5",
        pregunta: "¿Qué personaje es un astronauta?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/lomirqyjfditok8/veterinario.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/uwqkukvzmm4etv5/carpintera.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/3fmwluxxzxny6h3/astronauta.png?dl=1 ",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "6",
        pregunta: "¿Qué personaje es una maestra?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/sf6mr425pxj4uze/psicologo.jpg?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/e0cwyspav2lzwu7/maestra.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/99u7h87hmqjs5wx/mecanico.png?dl=1",
          },
        ],
      },
      {
        id: "7",
        pregunta: "¿Qué personaje es un veterinario?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/lomirqyjfditok8/veterinario.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/uyao5yvfvmnxcyo/alcalde.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/xip1r09aqhqtcbh/secretario.png?dl=1",
          },
        ],
      },
      {
        id: "8",
        pregunta: "¿Qué personaje es un abogado?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/w7mar8cmurdj89k/arquitecto.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/3fmwluxxzxny6h3/astronauta.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ul8f4tfi67neiw8/abogado.png?dl=1 ",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "9",
        pregunta: "¿Qué personaje es un agricultor?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/sf6mr425pxj4uze/psicologo.jpg?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ghfmqf6m0iisfq0/agricultor.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/7mp8an5gqs0xsya/policia.png?dl=1",
          },
        ],
      },
      {
        id: "10",
        pregunta: "¿Qué personaje es un albañil?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/cbqk03aig8w5hpz/alba%C3%B1il.png?dl=1 ",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/e0cwyspav2lzwu7/maestra.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/edkuf3jyt2phbda/bombero.png?dl=1",
          },
        ],
      },
      {
        id: "10",
        pregunta: "¿Qué personaje es un alcalde?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3oskfhgcskhmqol/farmaceutica.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/zoduuho2xls958y/sastre.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/uyao5yvfvmnxcyo/alcalde.png?dl=1 ",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "10",
        pregunta: "¿Qué personaje es un arquitecto?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/w7mar8cmurdj89k/arquitecto.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/qdgj3277oc5wn23/ingeniero.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/lomirqyjfditok8/veterinario.png?dl=1",
          },
        ],
      },
      {
        id: "11",
        pregunta: "¿Qué personaje es una carpintera?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ghfmqf6m0iisfq0/agricultor.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/xip1r09aqhqtcbh/secretario.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/uwqkukvzmm4etv5/carpintera.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "12",
        pregunta: "¿Qué personaje es una farmacéutica?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3oskfhgcskhmqol/farmaceutica.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/99u7h87hmqjs5wx/mecanico.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/e0cwyspav2lzwu7/maestra.png?dl=1",
          },
        ],
      },
      {
        id: "13",
        pregunta: "¿Qué personaje es un secretario?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/7mp8an5gqs0xsya/policia.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/3fmwluxxzxny6h3/astronauta.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/xip1r09aqhqtcbh/secretario.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "14",
        pregunta: "¿Qué personaje es un mecánico?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/7mp8an5gqs0xsya/policia.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/99u7h87hmqjs5wx/mecanico.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/ul8f4tfi67neiw8/abogado.png?dl=1",
          },
        ],
      },
      {
        id: "14",
        pregunta: "¿Qué personaje es un ingeniero?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/qdgj3277oc5wn23/ingeniero.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/cbqk03aig8w5hpz/alba%C3%B1il.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/zoduuho2xls958y/sastre.png?dl=1",
          },
        ],
      },
      {
        id: "15",
        pregunta: "¿Qué personaje es un psicólogo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3oskfhgcskhmqol/farmaceutica.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/sf6mr425pxj4uze/psicologo.jpg?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/99u7h87hmqjs5wx/mecanico.png?dl=1",
          },
        ],
      },
    ],
  },
  {
    id: "Estaciones",
    Question: [
      {
        id: "1",
        pregunta: "¿El sol sale cuando es ...?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/493yscu5bw46qlo/verano.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/7tgd7m7j94dqyhz/invierno.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/md8h6uwhn28wecl/primavera.jpg?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Cuando hace frío es porque estamos en ...?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/0rqoc1ij6uav9cs/oto%C3%B1o.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/7tgd7m7j94dqyhz/invierno.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/493yscu5bw46qlo/verano.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Cuando las hojas se caen es porque estamos en ...?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/0rqoc1ij6uav9cs/oto%C3%B1o.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/md8h6uwhn28wecl/primavera.jpg?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/7tgd7m7j94dqyhz/invierno.png?dl=1",
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Cuando hay muchas flores es porque estamos en ...?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/7tgd7m7j94dqyhz/invierno.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/493yscu5bw46qlo/verano.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/md8h6uwhn28wecl/primavera.jpg?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
    ],
  },
  {
    id: "Habitaciones",
    Question: [
      {
        id: "1",
        pregunta: "¿Cúal es la comisaría?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/6mmexoa2uhofrfi/comisaria.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/7bg80reds6lerp5/hospital.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/1236gm49iqj3t2b/iglesia.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Cúal es el hospital",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/rk3bdia247qv8bf/estaciondetren.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/4x5pso1hyaba61q/cine.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/7bg80reds6lerp5/hospital.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Cúal es la iglesia?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/6qyk30sc4jd8c6h/restaurante.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/1236gm49iqj3t2b/iglesia.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/jzzzit4pdfhavug/gasolinera.png?dl=1",
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Cual es el cine?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/9oy77aud5mxdvke/Bancos.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/dn8suovmqiyk1xk/school.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/4x5pso1hyaba61q/cine.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "5",
        pregunta: "¿Cual es la estación del tren?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/pa6tdafnij1jsf4/biblioteca.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/rk3bdia247qv8bf/estaciondetren.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/94ylhvlqwb5hv6m/correos.png?dl=1",
          },
        ],
      },
      {
        id: "6",
        pregunta: "¿Cual es el restaurante?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/6qyk30sc4jd8c6h/restaurante.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/ioti6ov57g6drnj/museo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/pusrvted5wqvl0c/piscina.png?dl=1",
          },
        ],
      },
      {
        id: "7",
        pregunta: "¿Cual es la gasolinera?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/jzzzit4pdfhavug/gasolinera.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/6mmexoa2uhofrfi/comisaria.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/jd1ymtddb7edngi/parque.png?dl=1",
          },
        ],
      },
      {
        id: "8",
        pregunta: "¿Cual es el colegio?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/1236gm49iqj3t2b/iglesia.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/dn8suovmqiyk1xk/school.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/7bg80reds6lerp5/hospital.png?dl=1",
          },
        ],
      },
      {
        id: "8",
        pregunta: "¿Cual es la estación de bomberos?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/rk3bdia247qv8bf/estaciondetren.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/4x5pso1hyaba61q/cine.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/4wqv5s4m9ha0qi5/bomberos.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "8",
        pregunta: "¿Cual es la estación de correos?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/jzzzit4pdfhavug/gasolinera.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/94ylhvlqwb5hv6m/correos.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/6qyk30sc4jd8c6h/restaurante.png?dl=1",
          },
        ],
      },
      {
        id: "9",
        pregunta: "¿Cual es el banco?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/9oy77aud5mxdvke/Bancos.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/dn8suovmqiyk1xk/school.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ioti6ov57g6drnj/museo.png?dl=1",
          },
        ],
      },
      {
        id: "10",
        pregunta: "¿Cual es la piscina?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/7bg80reds6lerp5/hospital.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/1236gm49iqj3t2b/iglesia.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/pusrvted5wqvl0c/piscina.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "11",
        pregunta: "¿Cual es el parque?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/jd1ymtddb7edngi/parque.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/rk3bdia247qv8bf/estaciondetren.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/6mmexoa2uhofrfi/comisaria.png?dl=1",
          },
        ],
      },
      {
        id: "12",
        pregunta: "¿Cual es el museo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ioti6ov57g6drnj/museo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/dn8suovmqiyk1xk/school.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/6qyk30sc4jd8c6h/restaurante.png?dl=1",
          },
        ],
      },
    ],
  },
  {
    id: "Transporte",
    Question: [
      {
        id: "1",
        pregunta: "¿Cúales son los transportes aéreos?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/qlb05peo0p0w49o/autobus.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/mevh61vivr34038/avion.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/h6ianbbryvjpapu/barco.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/33ges1asibiqojp/tren.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/b2nahv0mxind82u/avioneta.jpg?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/pmvuisxgo8b4dxo/piraguas.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/mu2fkvs5wh8hs36/helicoptero.png?dl=1",
             sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/o5gow9ukjextqhj/motoacua.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/m02tk11wwcdt3ds/camion.png?dl=1",
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Cúal es un transporte acuático?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/v9pqkqu3plgtx8n/bote.jpg?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/mu2fkvs5wh8hs36/helicoptero.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/m02tk11wwcdt3ds/camion.png?dl=1",
          },
        ],
      },
      {
        id: "5",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/fzeysf5cegvmkd7/grua.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/pjrlcw8as8lgxhl/pesquero.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/egrsy2paugtn660/globoaereo.png?dl=1",
          },
        ],
      },
      {
        id: "6",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/zjop6lou4xfe6fk/cuatri.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/cd9u4icbvs3onnm/cohete.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/kcguzfzlr7gd6o8/lancha.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "7",
        pregunta: "¿Cúal es un transporte terrestre",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/o5gow9ukjextqhj/motoacua.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/6ti2i0o44ugbzec/tractor.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/cd9u4icbvs3onnm/cohete.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/u6a4wfqlq1lqoiz/canoa.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/egrsy2paugtn660/globoaereo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/b30bmkt5nhqcrga/moto.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "3",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/rv8f0lb6tk1ns5q/caballo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/ztppdd2pxpu6j54/submarino.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/mevh61vivr34038/avion.png?dl=1",
          },
        ],
      },
    ],
  },
  {
    id: "Rostro",
    Question: [
      {
        id: "1",
        pregunta: "¿Cuál es la boca?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/fgl7fl9z9z70pqp/bocas.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/48s9tucmyhplg0i/manos.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/o9z64bod41g495q/pie.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Cúales son los labios?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3s8oyq0s4f3zv9r/barriga.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/dsxz58lavmfny8i/mejilla.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/r7g4fc9faj1be88/labios.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Cúal es la nariz?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/1h9e19x0vb6w4mo/narizon.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/t8vjxu4ygksefz3/barbilla.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/qubi730cqrejzmz/frente.png?dl=1",
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Cuales son los ojos?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/qubi730cqrejzmz/frente.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/rdh2dpxwpj9hmzo/ojos.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/3rp2cq483etri3p/pierna.jpg?dl=1",
          },
        ],
      },
      {
        id: "5",
        pregunta: "¿Cúal es la ceja?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/t8wl0og4kba7iwi/cejas.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/o9z64bod41g495q/pie.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/qubi730cqrejzmz/frente.png?dl=1",
          },
        ],
      },
      {
        id: "6",
        pregunta: "¿Cúales es la pestaña?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/fiql3d1ybyrrd6e/mu%C3%B1eca.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/sfcx6f46nwh0wfl/pesta%C3%B1a.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/x1eehafiq2yzwp0/espalda.png?dl=1",
          },
        ],
      },
      {
        id: "7",
        pregunta: "¿Cúales es la barbilla?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/t8vjxu4ygksefz3/barbilla.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/yoei3hfyvlnzf2p/codo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/3rp2cq483etri3p/pierna.jpg?dl=1",
          },
        ],
      },
      {
        id: "8",
        pregunta: "¿Cúales es la mejilla?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/nqn5fpxjc2dgm7f/cuello.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/duv0rpiijki9sr3/dedo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/dsxz58lavmfny8i/mejilla.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
    ],
  },
  {
    id: "Sabores",
    Question: [
      {
        id: "1",
        pregunta: "¿Cuál tiene sabor dulce?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/tjpumnkjuj1ix39/caramelo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/n001qovks8hzwkr/limon.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/55fhjkbs1lhbftp/sal.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Cúales tiene sabor salado?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/q1l1uyxqte9v5rk/chocolate.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/n238newxikz5s0i/naranja.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/55fhjkbs1lhbftp/sal.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Cúal tiene sabor amargo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/k7kgysk27m0qxau/ciruela.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/hqgbhe5mzs3i5dm/cafe.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/khfb3vkdgpxa3l8/aceitunas.png?dl=1",
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Cuales tiene sabor acído?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/1xrdmm62kl2oxti/brocoli.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ki8dwb2s4yto0hv/helado.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/n001qovks8hzwkr/limon.png?dl=1 ",
          },
        ],
      },
    ],
  },
  {
    id: "Escolares",
    Question: [
      {
        id: "1",
        pregunta: "¿Cuáles son los útiles escolares?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/oja15zoxyzfjw1b/lapiz.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/rzfpnaf5v2218k3/jabon.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/5pvvlbtmnfxbhvg/cuchillo.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3u9qjjq50003g14/martillo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/azk4itbtmwtplv3/cepillo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/t1i12n7hyptsfjp/tajador.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "3",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/kkqpskz0en1jkeu/toalla.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/13bfrrzjmewczuy/pegamento.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/r88s25q5b7hynsl/taladro.png?dl=1",
          },
        ],
      },
      {
        id: "4",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3h1khwpl16bmxpq/borrador.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/4zyzquyc5wdx0qy/alcohol.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/l2430esattigwkz/cartulinas.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "5",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/rsi4zvlk84z3xom/jarra.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ggeif1mt8ng6tiu/talco.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/pt1x5gim19xod1r/acuarelas.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "6",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/d4b48b1ez5uh8ba/peine.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/7299030n97qa5hm/enchufe.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/cd4zt7rkt6gka52/mochila.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
    ],
  },
  {
    id: "Aseo",
    Question: [
      {
        id: "1",
        pregunta: "¿Cuáles son los útiles de aseo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/oja15zoxyzfjw1b/lapiz.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/rzfpnaf5v2218k3/jabon.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/5pvvlbtmnfxbhvg/cuchillo.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3u9qjjq50003g14/martillo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/azk4itbtmwtplv3/cepillo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/t1i12n7hyptsfjp/tajador.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/kkqpskz0en1jkeu/toalla.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/13bfrrzjmewczuy/pegamento.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/6qqx5v7aq0lmyy6/cremas.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "4",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3h1khwpl16bmxpq/borrador.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/4zyzquyc5wdx0qy/alcohol.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/kt4y93oxqwat7dr/pastadental.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "5",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/rsi4zvlk84z3xom/jarra.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ggeif1mt8ng6tiu/talco.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/pt1x5gim19xod1r/acuarelas.png?dl=1",
          },
        ],
      },
      {
        id: "6",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/d4b48b1ez5uh8ba/peine.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/7299030n97qa5hm/enchufe.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/27768tao4opii06/acondicionador.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
    ],
  },
  {
    id: "Peligroso",
    Question: [
      {
        id: "1",
        pregunta: "¿Cuáles son los objetos peligrosos?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/t24mddlqy6jpnn8/plancha.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/rzfpnaf5v2218k3/jabon.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/5pvvlbtmnfxbhvg/cuchillo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "2",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3u9qjjq50003g14/martillo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/azk4itbtmwtplv3/cepillo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/pysr7tx7nj1vuak/agenda.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/kkqpskz0en1jkeu/toalla.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/4zyzquyc5wdx0qy/alcohol.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/r88s25q5b7hynsl/taladro.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "4",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/3h1khwpl16bmxpq/borrador.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/vqne12gno75kult/alambre.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/l2430esattigwkz/cartulinas.png?dl=1",
          },
        ],
      },
      {
        id: "5",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/7299030n97qa5hm/enchufe.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/ggeif1mt8ng6tiu/talco.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/xbc2vum4qy4arsr/alicate.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "6",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/d4b48b1ez5uh8ba/peine.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/5yrynh1t9d9ye1c/aguja.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/cd4zt7rkt6gka52/mochila.png?dl=1",
          },
        ],
      },
    ],
  },
  {
    id: "Semáforo",
    Question: [
      {
        id: "1",
        pregunta: "¿Qué color es para detenerse?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/74os7w2osplv8px/semaforojo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/emghaz3vid4lnsq/semaforoambar.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/h0ou18ix82ixrp4/semaforoverde.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Qué color es de precaución?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/74os7w2osplv8px/semaforojo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/emghaz3vid4lnsq/semaforoambar.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/h0ou18ix82ixrp4/semaforoverde.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Qué color es para seguir?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/74os7w2osplv8px/semaforojo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/emghaz3vid4lnsq/semaforoambar.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/h0ou18ix82ixrp4/semaforoverde.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
    ],
  },
  {
    id: "Sentidos",
    Question: [
      {
        id: "1",
        pregunta: "¿Qué sentido se usa para saborear?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/degt3779h6u4ddn/olfato.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ev0tof62katkb2b/gustos.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/s15v0gxeemo8rng/ojo.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Qué sentido se usa para mirar?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/s15v0gxeemo8rng/ojo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/ize7sozg8saj8hw/o%C3%ADdo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/yr4bs9q1uuu0bps/mano.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Qué sentido se usa para oler?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ize7sozg8saj8hw/o%C3%ADdo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/degt3779h6u4ddn/olfato.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/ev0tof62katkb2b/gustos.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Qué sentido se usa para tocar?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/ev0tof62katkb2b/gustos.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/yr4bs9q1uuu0bps/mano.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/s15v0gxeemo8rng/ojo.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Qué sentido se usa para oír?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/yr4bs9q1uuu0bps/mano.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/degt3779h6u4ddn/olfato.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ize7sozg8saj8hw/o%C3%ADdo.png?dl=1 ",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
    ],
  },
  {
    id: "Espaciales",
    Question: [
      {
        id: "1",
        pregunta: "Señala la pelota que se encuentra adelante",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/73pziy50i4wbnsm/delante.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/kv803yk768lba91/detras.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/65g53wnrknhz7er/arriba.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "Señala la pelota que se encuentra atrás",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/65g53wnrknhz7er/arriba.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/kv803yk768lba91/detras.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/73pziy50i4wbnsm/delante.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "Señala la pelota que se encuentra arriba",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/s2b9eoce0jwwrvx/abajo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/73pziy50i4wbnsm/delante.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/65g53wnrknhz7er/arriba.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "4",
        pregunta: "Señala la pelota que se encuentra abajo",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/73pziy50i4wbnsm/delante.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/s2b9eoce0jwwrvx/abajo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/kv803yk768lba91/detras.png?dl=1",
          },
        ],
      },
      {
        id: "5",
        pregunta: "Señala la pelota que se encuentra dentro",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/gt6jh9jwdgmmr3j/dentro.png?dl=1",
             sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/s2b9eoce0jwwrvx/abajo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/zpydsbkppclx9rh/fuera.png?dl=1",
          },
        ],
      },
      {
        id: "6",
        pregunta: "Señala la pelota que se encuentra afuera",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/65g53wnrknhz7er/arriba.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/zpydsbkppclx9rh/fuera.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/gt6jh9jwdgmmr3j/dentro.png?dl=1",
          },
        ],
      },
      {
        id: "7",
        pregunta: "Señala la flecha que va a la izquierda",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/khi4z8wbj05tv8a/derecha.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/gt6jh9jwdgmmr3j/dentro.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/xyfhnkqnp33gs2n/izquierda.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "8",
        pregunta: "Señala la flecha que va a la derecha",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/khi4z8wbj05tv8a/derecha.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/xyfhnkqnp33gs2n/izquierda.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/gt6jh9jwdgmmr3j/dentro.png?dl=1",
          },
        ],
      },
    ],
  },
  {
    id: "Geometricas",
    Question: [
      {
        id: "1",
        pregunta: "¿Cuál figura geométrica es un triángulo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/z8c7iwlxd9apiow/triangulo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/y2izzs7vfn01y2d/circulo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/t1v3qfjosa212cy/rectangulo.png?dl=1",
          },
        ],
      },
      {
        id: "2",
        pregunta: "¿Cuál figura geométrica es un círculo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/y2izzs7vfn01y2d/circulo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/t1v3qfjosa212cy/rectangulo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/z8c7iwlxd9apiow/triangulo.png?dl=1",
          },
        ],
      },
      {
        id: "3",
        pregunta: "¿Cuál figura geométrica es un rectángulo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/t1v3qfjosa212cy/rectangulo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/z8c7iwlxd9apiow/triangulo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/y2izzs7vfn01y2d/circulo.png?dl=1",
          },
        ],
      },
      {
        id: "4",
        pregunta: "¿Cuál figura geométrica es un cuadrado?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/08s05mdll51llh5/ovalo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ofq808wdlkutx3v/rombo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/33xr7tyudtcs08y/cuadrado.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "5",
        pregunta: "¿Cuál figura geométrica es un rombo?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/33xr7tyudtcs08y/cuadrado.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/08s05mdll51llh5/ovalo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/ofq808wdlkutx3v/rombo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
        ],
      },
      {
        id: "6",
        pregunta: "¿Cuál figura geométrica es un ovalado?",
        imagenes: [
          {
            uri: "https://www.dropbox.com/s/08s05mdll51llh5/ovalo.png?dl=1",
            sonido: require("../../assets/Audios/correcto.mp3"),
          },
          {
            uri: "https://www.dropbox.com/s/ofq808wdlkutx3v/rombo.png?dl=1",
          },
          {
            uri: "https://www.dropbox.com/s/33xr7tyudtcs08y/cuadrado.png?dl=1", 
          },
        ],
      },
    ],
  },
];
